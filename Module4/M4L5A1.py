# M4L5A1: School Store Inventory Checker
# Activity 1: Advanced Python Functions - List Comprehension, Dict Comprehension, map(), zip(), exit()

# PART 1: Create the store's item names and stock counts
items = ["pencil", "eraser", "notebook", "sharpener", "glue"]
stock_counts = [12, 0, 8, 5, 3]

# PART 2: Pair items with stock counts into a dictionary using zip() and dict comprehension
inventory = {item: count for item, count in zip(items, stock_counts)}
print("Full Inventory:", inventory)

# PART 3: Filter only the items that are still in stock using list comprehension
...
# PART 5: Stop the checker early if the chosen item is out of stock using exit()
